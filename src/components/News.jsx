          ...
          {items.map((item, i) => (
            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-[0.65rem] text-lab-muted shrink-0">
              ...
              {item.url ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="text-xs text-gray-800 leading-relaxed line-clamp-3 hover:text-lab-blue transition-colors"
                >
                  {item.body}
                </a>
              ) : (
...
export function News() {
  ...
  return (
    <section className="px-[5%] py-14 bg-white min-h-[60vh]">
      <div className="max-w-[760px] mx-auto">
        <Reveal>
          <SectionLabel text="Latest Updates" />
          <SectionTitle>Recent News</SectionTitle>
          <p className="text-sm text-lab-muted leading-loose mb-8 max-w-[560px]">
            Papers, events, and highlights from the IRoHS Lab.
          </p>
        </Reveal>

        {loading && (
          <div className="flex items-center gap-2 text-sm text-lab-muted py-10">
        ...
        {!loading && !error && (
          <>
                  ...
                  <button
                    ...
                    className={`text-xs font-semibold px-3.5 py-1.5 rounded-full border transition-all ${
                      active === tag
                        ...
                        : "bg-white text-[#555] border-lab-border hover:border-lab-blue hover:text-lab-blue"
                    }`}
                  >
                    {tag}
                  </button>
            ...
            {/* Timeline */}
            <ol className="relative border-l border-lab-border ml-3">
              {filtered.map((item, i) => (
                <Reveal key={i} delay={i * 40}>
                  <li className="mb-8 pl-6 relative">
                    <span className="absolute -left-[7px] top-1.5 w-3.5 h-3.5 rounded-full bg-white border-2 border-lab-blue" />

                    <time className="block text-[0.68rem] font-semibold text-lab-muted tracking-wide uppercase mb-1.5">
                      {formatDate(item.date)}
                    </time>

                    <div className="bg-white border border-lab-border rounded-[10px] px-4 py-3.5 hover:border-lab-blue hover:shadow-card-hover transition-all group">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        ...
                        {item.url && (
                          <span className="text-[#ccc] text-xs group-hover:text-lab-blue transition-colors shrink-0">↗</span>
                        )}
                      </div>

                      {item.url ? (
                        <a href={item.url} target="_blank" rel="noreferrer" className="block">
                          <p className="text-sm text-gray-800 leading-relaxed group-hover:text-lab-blue transition-colors">
                            {item.body}
                          </p>
                        </a>
                      ) : (
              ...
              {filtered.length === 0 && (
                <li className="pl-6 text-sm text-lab-muted py-6">No items in this category.</li>
              )}
            </ol>
          </>
        )}
